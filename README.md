...
统一返回格式：`{"code":200, "message":"操作成功", "data":...}`。认证、鉴权、路径、请求体这四类错误使用真实 HTTP 状态码（`401` / `403` / `404` / `400`），响应体仍是同一格式；路径不存在返回 `404` 而不是被兜底成"系统异常"。
...
- 接口错误分两类约定，前端需要按两种方式判断：
  ...
  - **业务层失败走 HTTP 200 + `code:500`**：参数校验失败、登录账号已存在、用户不存在等
  - 两者响应体都是统一的 `{"code":...,"message":...,"data":...}`。要把业务失败也统一成真实状态码，改动集中在 `GlobalExceptionHandler`
